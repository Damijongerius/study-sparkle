...
router.post('/signup', async (req, res) => {
  try {
    ...
    res.json({ success: true, user: { id: user._id, username: user.username, friendCode: user.friendCode }, friends: [] });
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ success: true, user: { id: user._id, username: user.username, friendCode: user.friendCode }, friends: data?.friends || [] });
  } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
...
router.get('/me', async (req, res) => {
  ...
  const data = await UserData.findOne({ userId: user._id });
  res.json({ user: { id: user._id, username: user.username, friendCode: user.friendCode }, friends: data?.friends || [] });
});
